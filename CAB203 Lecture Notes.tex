...
IF .. THEN is a binary operator that results in 1 if \(y\) is 1 when \(x\) is also 1.
When \(x\) is 0, the result is always 1.
\begin{table}[h]
    \centering
...
IF AND ONLY IF is a binary operator that results in 1 if \textbf{both} operands are the same value.
\begin{table}[h]
    \centering
